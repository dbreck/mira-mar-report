...
6. **Never reload the tab once ranges are set.** A `navigate`-by-URL (or browser reload) resets EVERY page's date filter back to the report default (`Last 30 days`, which drops the first day of a custom range). Move between pages only by clicking the left sidebar (`find` the treeitem, then `left_click` by ref). Learned 2026-09-14: one URL navigate silently wiped all 8 hand-set ranges; the tell was Google Ads Overview reading Aug 15 – Sep 13 after the user had set Aug 14. If it happens, re-set each page yourself — the picker is 3 clicks: date control (top-right, ~1230×131 at 1432 wide) → start-day cell → Apply.
7. **Virtualized tables load ~12 rows at a time.** Google Ads Insights (forms / calls), Meta Lead Insights (56 rows) and the GA sessions table only render a window; scroll INSIDE the widget (cursor over the table) in 3–4-tick steps and `get_page_text` after each — 6-tick steps skip rows. Confirm coverage by summing the captured rows against the widget's own `1 - N / N` footer.
8. **CallRail page status (2026-09-14):** the Total / 1st Time / Answered KPI tiles, the Calls-by-Source bar and Answered-by-Tracking-Number are ALL "Unknown data source" now (connector broken further since Aug). The daily time-series table, its summary tiles and the Source table still load and reconcile — use those; note the two Average Duration tiles differ (mean-of-daily-averages incl. zero days vs per-call mean).
